...
const AddPets = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const productData = new FormData(); // Create FormData instance

        for (const key in form) {
            if (Array.isArray(form[key])) {
                form[key].forEach((file) => productData.append(key, file)); // Append files
            } else {
                productData.append(key, form[key]); // Append other data
            }
        }
        console.log(form)
    ...
    //         animalType: product.animalType,
    //         breadType: product.breadType,
    //         image: product.image, // Update logic needed here for file uploads
    //         gender: product.gender,
    //         age: product.age,
    //         ageUnit: product.age,
    //         size: product.ageUnit,
